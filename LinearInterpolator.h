...
public:
  ...
  Number_Type operator()(const Number_Type x) const;

private:
  const std::vector<Number_Type> x_points; 	/* Must be increasing sequence, i.e. x[i] < x[i+1] */
...
template <typename Number_Type>
Number_Type LinearInterpolator<Number_Type>::operator()(const Number_Type x) const
{
  Number_Type res = 0.;
