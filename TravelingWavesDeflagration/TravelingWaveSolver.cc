...
namespace TravelingWave
{
  ...
  TravelingWaveSolver::TravelingWaveSolver(const Parameters &parameters, const SolutionStruct &initial_guess_input) 
    ...
    , fe(FE_Q<1>(params.fe.poly_degree), 1, 
          FE_Q<1>(params.fe.poly_degree), 1)   // 2 fe basis sets, corresponding to dT, dlambda
    ...
    , computing_timer(std::cout, TimerOutput::never, TimerOutput::wall_times)
  {
    ...
    table.add_value("Parameter name", "T_0r");
    table.add_value("value", params.problem.T_right);
  ...
  void TravelingWaveSolver::find_boundary_and_centering_dof_numbers()
  {
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      for (const auto &v_ind : cell->vertex_indices())
      {
        ...
        {
          boundary_and_centering_dof_numbers["T_left"]        = cell->vertex_dof_index(v_ind, 0);
          boundary_and_centering_dof_numbers["lambda_left"]   = cell->vertex_dof_index(v_ind, 1);
        }
        else if (isapprox(cell->vertex(v_ind)[0], params.mesh.interval_right))
        {
          boundary_and_centering_dof_numbers["T_right"]       = cell->vertex_dof_index(v_ind, 0);
          boundary_and_centering_dof_numbers["lambda_right"]  = cell->vertex_dof_index(v_ind, 1);
        }
        else if (isapprox(cell->vertex(v_ind)[0], 0.))
        {
          boundary_and_centering_dof_numbers["T_zero"]        = cell->vertex_dof_index(v_ind, 0);
        }
      }
    }
  }
  ...
  void TravelingWaveSolver::set_boundary_and_centering_values()
  {
    ...
    current_solution[boundary_and_centering_dof_numbers["T_zero"]] = problem.T_ign;
		...
		current_solution[boundary_and_centering_dof_numbers["lambda_left"]]  = 1.;
  ...
  void TravelingWaveSolver::setup_system(const bool initial_step)
  {
    ...
    // Dirichlet homogeneous boundary conditions for $d\lambda$.
		zero_boundary_constraints.add_line(boundary_and_centering_dof_numbers["lambda_left"]);
  ...
  void TravelingWaveSolver::set_initial_guess()
  {
    ...
    // The initial condition is a discrete set of coordinates $\xi$ and values of functions $T$ and $\lambda$. From the three sets we create three continuous functions using interpolation, which then form one continuous vector function of <code> SolutionVectorFunction </code> type.
    ...
    SolutionVectorFunction init_guess_func(T_interpolant, lambda_interpolant);
  ...
  {
    {
      ...
      for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        auto sqrt_term = [=](double T){
          return std::sqrt(1 - std::pow(problem.epsilon, 2) * T + 2 * problem.epsilon * problem.u_right 
                  + std::pow(problem.epsilon, 2) * (std::pow(problem.u_right, 2) + problem.T_right));
        };

        auto alpha = [=](double T){
          return 1 + problem.epsilon / (2 * sqrt_term(T));
        };

        auto beta = [=](double T){
          return std::pow(problem.epsilon / sqrt_term(T), 3) / 4;
        };
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i)
          {
            for (unsigned int j = 0; j < dofs_per_cell; ++j)
            {
              cell_matrix(i, j) += (
                ...
                + phi_T[i] * (
                  wave_speed * grad_phi_T[j][0] * alpha(current_temperature_values[q])
                  + wave_speed * current_temperature_gradients[q][0] * beta(current_temperature_values[q]) * phi_T[j]
                ...
                + 1/problem.Le * (-grad_phi_lambda[i] * grad_phi_lambda[j])
                + phi_lambda[i] * (
            ...
            row_last_element_vector(i) += (
              (phi_T[i] * current_temperature_gradients[q][0] * alpha(current_temperature_values[q]))
      ...
      double lambda_at_zero_point(0.);
      {
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
        {
          for (const auto &vertex : cell->vertex_indices())
          {
            ...
            {
              T_zero_point_dof_ind = cell->vertex_dof_index(vertex, 0);
              lambda_zero_point_dof_ind = cell->vertex_dof_index(vertex, 1);
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
      {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          cell_residual(i) += (
            ...
            (-grad_phi_T[i] * current_temperature_gradients[q])
            + phi_T[i] * (
              wave_speed * current_temperature_gradients[q][0] * alpha(current_temperature_values[q])
              + problem.q * omega(current_temperature_values[q], current_lambda_values[q])
            )

            + 1/problem.Le * (-grad_phi_lambda[i] * current_lambda_gradients[q])
            + phi_lambda[i] * (
...
#  endif
  ...
  void TravelingWaveSolver::output_with_double_precision(const Vector<double> &solution, const double wave_speed, const std::string filename)
  {
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      for (const auto &v_ind : cell->vertex_indices())
      {
        double T = solution(cell->vertex_dof_index(v_ind, 0));
        double lambda = solution(cell->vertex_dof_index(v_ind, 1));

        auto sqrt_term = [=](double T){
          return std::sqrt(1 - std::pow(problem.epsilon, 2) * T + 2 * problem.epsilon * problem.u_right 
                  + std::pow(problem.epsilon, 2) * (std::pow(problem.u_right, 2) + problem.T_right));
          
          // Expansion of sqrt_term function up to the fourth order of epsilon (just in case):
          // 1 + epsilon*u_r + epsilon**2*(T_{0r}/2 - \Theta_{0}/2) + epsilon**3*(u_r**3/2 - u_r*(T_{0r} - \Theta_{0} + u_r**2)/2) + O(epsilon**4)

          // It is better to compute the value using the expansion formula for small values of epsilon.
        };
        auto u_func = [=](double T){
          if (problem.epsilon > 0.)
          {
            return (-1 + sqrt_term(T)) / problem.epsilon;

            // Expansion of U_0 function up to the third order of epsilon (just in case):
            // u_r + epsilon*(T_{0r}/2 - \Theta_{0}/2) + epsilon**2*(u_r**3/2 - u_r*(T_{0r} - \Theta_{0} + u_r**2)/2) + O(epsilon**3)
        ...
        output << std::scientific << std::setprecision(16);
        output << std::scientific << " " << u_func(T) << " " << T << " " << lambda << "\n";
      }
      output  << "\n";
    }
  ...
  void TravelingWaveSolver::get_solution(SolutionStruct &solution) const
  {
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
      for (const auto &v_ind : cell->vertex_indices())
      {
        ...
        double T = current_solution(cell->vertex_dof_index(v_ind, 0));
        double lambda = current_solution(cell->vertex_dof_index(v_ind, 1));
        solution_set.insert({x, T, lambda});
      }
    }
    ...
    for (auto it = solution_set.begin(); it != solution_set.end(); ++it)
    {
      ...
      solution.T.push_back((*it)[1]);
      solution.lambda.push_back((*it)[2]);
    }
